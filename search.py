...
class AutoCompleteData:
    def __init__(self, item):
        ...
        self.source_text = f"{ntpath.basename(file_dict[item[1]])[:-4]} {item[2]}"
        self.offset = item[4]
        self.score = item[3]

    def print(self):
        print(f"complete sentence: {self.completed_sentence} ({self.source_text}) offset: {self.offset}")
...
def is_in_dict(term, new_term, score):
    ...
    if tmp != None:
        tmp=tuple(set(tmp))
        for tup in tmp:
            line = linecache.getline(file_dict[tup[0]], tup[1])[:-1].lstrip()
            if term != new_term:
                if term not in line:
                    #res_list.append(tuple([line, tup[0], tup[1], score, line.lower().find(new_term)]))
                    res_list.append(tuple([line, tup[0], tup[1], score,fix_term(line).find(new_term)]))
            else:
                #res_list.append(tuple([line, tup[0], tup[1], score, line.lower().find(new_term)]))
                res_list.append(tuple([line, tup[0], tup[1], score, fix_term(line).find(new_term)]))
    return res_list
...
def search(term):
    term = fix_term(term)
    perfect_list = check_perf(term)
    term_list = perfect_list
